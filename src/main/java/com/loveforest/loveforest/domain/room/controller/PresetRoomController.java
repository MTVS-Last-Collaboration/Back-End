...
public class PresetRoomController {
    ...
    @Operation(
            summary = "방 프리셋 적용",
            description = "특정 방에 프리셋을 적용하여 방의 상태(벽지, 바닥, 가구 배치 등)를 설정합니다."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "프리셋 적용 성공",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = PresetRoomResponseDTO.class),
                            examples = @ExampleObject(value = """
            ...
            @ApiResponse(
                ...
                {
                    "status": 404,
                    "errorType": "RoomOrPresetNotFound",
                    "message": "해당 방이나 프리셋을 찾을 수 없습니다.",
                    "code": "ROOM-PRESET-001"
                }
                """
                            )
                    )
